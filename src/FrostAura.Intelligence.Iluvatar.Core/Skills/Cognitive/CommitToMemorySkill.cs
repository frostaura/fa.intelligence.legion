...
namespace FrostAura.Intelligence.Iluvatar.Core.Skills.Cognitive
{
    ...
    public class CommitToMemorySkill : BaseSkill
    {
        ...
        public CommitToMemorySkill(IOptions<PineconeConfig> pineconeOptions, EmbeddingSkill embeddingSkill, ILogger<CommitToMemorySkill> logger)
            : base(logger)
        {
        ...
        private async Task<List<Memory>> GetMemoriesFromTextBlobAsync(string text, Dictionary<string, string> context, CancellationToken token)
        {
            ...
            for (int i = 0; i < response.Length; i++)
            {
                ...
                memory.Metadata.Id = Guid.NewGuid().ToString();
            }
